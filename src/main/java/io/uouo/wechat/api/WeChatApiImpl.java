...
public class WeChatApiImpl implements WeChatApi {
    ...
    private boolean processLoginSession(String loginContent) {
        ...
        // UOS 协议（2021 年后）：新版登录响应不再以 XML 返回 skey/wxsid/wxuin/pass_ticket，
        // 凭据改从响应 Cookie 中获取；skey 置空、pass_ticket 用 deviceid 代替。
        // 这里先尝试 XML 解析（兼容老协议），取不到再走 UOS 逻辑。
        String skey = WeChatUtils.match("<skey>(\\S+)</skey>", body);
        String wxSid = WeChatUtils.match("<wxsid>(\\S+)</wxsid>", body);
        String wxUin = WeChatUtils.match("<wxuin>(\\S+)</wxuin>", body);
        String passTicket = WeChatUtils.match("<pass_ticket>(\\S+)</pass_ticket>", body);

        if (StringUtils.isEmpty(skey)) {
            skey = "";
        }
        if (StringUtils.isEmpty(wxSid)) {
            wxSid = this.client.cookie("wxsid");
        }
        if (StringUtils.isEmpty(wxUin)) {
            wxUin = this.client.cookie("wxuin");
        }
        if (StringUtils.isEmpty(passTicket)) {
            passTicket = loginSession.getDeviceId();
        }
        if (StringUtils.isEmpty(wxSid) || StringUtils.isEmpty(wxUin)) {
            log.error("登录失败：未能从登录响应/Cookie 中获取会话凭据（wxsid={}, wxuin={}），请检查网络或账号网页版登录权限", wxSid, wxUin);
        }

        loginSession.setSKey(skey);
        loginSession.setWxSid(wxSid);
        loginSession.setWxUin(wxUin);
        loginSession.setPassTicket(passTicket);
    ...
    public void loadContact(int seq) {
        ...
        while (true) {
            ...
            for (Account account : memberList) {
                // 修复原版笔误：原本只有 userName 为 null 时才放入联系人表，
                // 导致 accountMap 永远为空、好友过滤(accountType)失效、联系人显示 0。
